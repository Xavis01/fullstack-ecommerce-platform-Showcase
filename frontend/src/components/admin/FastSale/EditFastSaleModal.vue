            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl p-6 relative font-montserrat mb-auto"
                ...
                <div v-if="isLoadingData"
                    ...
                    <div class="text-center">
                        <LoaderCircle class="w-12 h-12 animate-spin text-[#0f2301] mx-auto mb-2" />
                        <p class="text-[#3a5528] font-montserrat italic">Carregando dados...</p>
                    </div>
                </div>

                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-2xl font-light italic text-[#3a5528]">Editar Venda #{{ sale?.id }}</h2>
                    <div class="text-[#3a5528] mr-8">
                        <span class="font-bold text-sm text-[#0f2301]/70 block">Vendedor</span>
                        <span class="font-light italic">{{ sale?.created_by_name || sale?.created_by }}</span>
                    </div>
                </div>
                ...
                <div class="bg-[#e4e3db] p-4 rounded-lg border border-[#0f2301]/10 mb-6">
                    <h3 class="text-lg font-light italic text-[#3a5528] mb-4">Adicionar/Editar Itens</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="h-11 flex items-center ml-4">
                                <QuantityControl v-model="quantity" :min="1"
                                    :max="selectedVariant ? getEffectiveStock(selectedVariant) : 1"
                ...
                <div v-if="form.items.length > 0" class="mb-6">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in form.items" :key="index" class="border-b border-[#0f2301]/10">
                                <td class="p-3">
                                    <img v-if="item.product_image" :src="getImageUrl(item.product_image)"
                                        class="w-12 h-12 object-cover rounded" />
                                    <div v-else class="w-12 h-12 bg-[#0f2301] rounded flex items-center justify-center">
                                ...
                                <td class="p-3">
                                    <button @click="removeItem(index)" class="text-[#9a382d] hover:text-[#9a382d]/80">
...
import { X, Plus, LoaderCircle, Trash2, QrCode, Banknote, CreditCard, User, Handshake } from 'lucide-vue-next'
import api from '@/api'
...
// Armazena a quantidade original de cada variante nesta venda para cálculo de estoque efetivo
...
const variantOptions = computed(() => {
    return availableVariants.value.map(v => ({
        value: v.id,
        label: `${v.size} (Estoque: ${getEffectiveStock(v)})`,
        disabled: getEffectiveStock(v) <= 0
    }))
})
...
// Calcula estoque efetivo = estoque atual no banco + o que está "preso" nesta venda (originalmente)
// E subtrai o que já está na lista atual do formulário (se não for o item que estamos editando - mas aqui só adicionamos items novos na logica "add to list")
// Na verdade, a lista `form.items` RECRIA a venda. Então tudo na lista consome estoque.
// O estoque disponível para adicionar é: (estoque_db + qtd_original_dessa_venda) - qtd_já_na_lista
const getEffectiveStock = (variant) => {
    const originalQty = originalQuantities.value[variant.id] || 0
    const currentQtyInForm = form.value.items
        .filter(i => i.product_variant_id === variant.id)
        .reduce((sum, i) => sum + i.quantity, 0)

    return (variant.stock + originalQty) - currentQtyInForm
}
...
const isValidItem = computed(() => {
    if (!selectedProduct.value || !selectedVariant.value || quantity.value <= 0) return false
    return quantity.value <= getEffectiveStock(selectedVariant.value)
})
...
const addItem = () => {
    ...
    if (existingIndex >= 0) {
        const maxAllowed = getEffectiveStock(variant)
        if (form.value.items[existingIndex].quantity + quantity.value > maxAllowed) {
...
const submit = async () => {
    ...
    try {
        ...
        await api.put(`/fast-sales/${props.sale.id}`, form.value)
...
// When sale prop changes, populate the form
watch(() => props.sale, (newSale) => {
    if (newSale) {
        isLoadingData.value = true
        form.value.client_name = newSale.client_name || ''
        form.value.pay_method = newSale.pay_method
        form.value.date = newSale.date
        form.value.items = newSale.items ? JSON.parse(JSON.stringify(newSale.items)) : []

        // Map original quantities for stock calculation
        originalQuantities.value = {}
        newSale.items.forEach(item => {
            if (!originalQuantities.value[item.product_variant_id]) {
                originalQuantities.value[item.product_variant_id] = 0
            }
            originalQuantities.value[item.product_variant_id] += item.quantity
        })
