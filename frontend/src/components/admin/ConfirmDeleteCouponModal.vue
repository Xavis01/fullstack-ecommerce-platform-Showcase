...
async function deleteCoupon() {
    ...
    loading.value = true
    try {
        await api.delete(`/admin/coupons/delete/${props.coupon.id}`)
        toast.success('Cupom removido com sucesso')
        emit('deleted')
    } catch (err) {
        const msg = err.response?.data?.error || 'Erro ao remover cupom'
        toast.error(msg)
        console.error(err)
    } finally {
        loading.value = false
        ...
        <div v-if="isOpen" @click="emit('close')"
            ...
            <div @click.stop
                ...
                <h2 class="text-xl font-light italic text-[#3a5528] mb-2 text-center tracking-wide">
                    Deseja realmente remover o cupom <span class="font-semibold">"{{ coupon?.nome }}"</span>?
                </h2>
                <p
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <button @click="deleteCoupon" :disabled="loading"
