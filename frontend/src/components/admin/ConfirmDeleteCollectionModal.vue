...
async function deleteCollection() {
    ...
    loading.value = true
    try {
        await api.delete(`/admin/collections/delete/${props.collection.id}`)
        toast.success('Coleção removida com sucesso')
        emit('deleted')
    } catch (err) {
        const msg = err.response?.data?.error || 'Erro ao remover coleção'
        ...
        <div v-if="isOpen" @click="emit('close')"
            ...
            <div @click.stop
                ...
                <h2 class="text-xl font-light italic text-[#3a5528] mb-2 text-center tracking-wide">
                    Deseja realmente remover a coleção <span class="font-semibold">"{{ collection?.name }}"</span>?
                </h2>
                <p
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <button @click="deleteCollection" :disabled="loading"
                        class="px-6 py-2.5 bg-[#9a382d] text-[#fffdf2] rounded-lg hover:bg-[#9a382d]/90 transition-all disabled:opacity-50 italic tracking-wide flex items-center gap-2">
