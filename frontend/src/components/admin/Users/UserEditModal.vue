        ...
        <div v-if="isOpen" @click="closeModal"
            ...
            <div @click.stop
                ...
                <div class="bg-[#0f2301] px-8 py-5 flex items-center gap-3">
                    <UserCog class="text-[#fffdf2]" :size="20" />
                    <h2 class="text-lg font-['Montserrat'] font-light italic tracking-widest text-[#fffdf2] uppercase">
                        Editar Usuário
                    </h2>
                    <button @click="closeModal"
                        class="ml-auto text-[#fffdf2]/60 hover:text-[#fffdf2] transition-colors">
                        <X :size="20" />
                    </button>
                </div>
                ...
                <div class="px-8 py-6 space-y-5">
                    ...
                    <!-- Nova Senha (Opcional) -->
                    <div>
                        <label class="label">Nova Senha <span class="text-[#8c9e78] text-xs">(Opcional)</span></label>
                        <div class="relative">
                    ...
                    <div class="flex items-center gap-3 pt-1">
                        <button @click="form.is_admin = !form.is_admin"
                            ...
                            <Check v-if="form.is_admin" class="text-[#fffdf2]" :size="12" />
                        </button>
...
import api from '@/api'
import { X, UserCog, Check, Loader2, Eye, EyeOff } from 'lucide-vue-next'
...
async function submit() {
    ...
    saving.value = true
    try {
        await api.put(`/admin/users/update/${props.user.id}`, form.value)
        toast.success('Usuário atualizado com sucesso!')
        emit('updated')
        closeModal()
    } catch (e) {
        toast.error(e.response?.data?.error || 'Erro ao atualizar usuário.')
    } finally {
        saving.value = false
    }
}
